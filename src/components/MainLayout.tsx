...
const MainLayout: React.FC<MainLayoutProps> = ({ children }) => {
  ...
  const handleDownloadApp = async () => {
    try {
      ...
      // First, let's try to list files to see what's available
      ...
      if (listError) {
        ...
        const { data: altFiles, error: altListError } = await supabase.storage
          .from('apkfiles')
          .list('', {
            limit: 100,
            offset: 0
        ...
        if (altListError) {
          console.error('Error with alternative bucket:', altListError);
          toast({
            title: "Download Error",
            description: "Could not access APK files. Please contact support.",
            variant: "destructive",
          });
          return;
        }

        // Use alternative bucket
        console.log('Available files in apkfiles bucket:', altFiles);
        
        // Look for APK file
        const apkFile = altFiles?.find(file => file.name.endsWith('.apk'));
        if (!apkFile) {
          toast({
            title: "Download Error",
            description: "APK file not found in storage.",
            variant: "destructive",
          });
          return;
        }

        // Get signed URL from alternative bucket
        const { data, error } = await supabase.storage
          .from('apkfiles')
          .createSignedUrl(apkFile.name, 3600);
      ...
      // Original bucket worked, proceed normally
      console.log('Available files:', files);
      
      // Look for APK file
      const apkFile = files?.find(file => file.name.endsWith('.apk')) || files?.[0];
      if (!apkFile) {
        toast({
          title: "Download Error",
          description: "APK file not found in storage.",
      ...
      const { data, error } = await supabase.storage
        .from('axiom-smart-track.apk')
        .createSignedUrl(apkFile.name, 3600); // 1 hour expiry
