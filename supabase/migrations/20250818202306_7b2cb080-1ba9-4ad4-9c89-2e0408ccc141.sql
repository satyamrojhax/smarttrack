...
DROP POLICY IF EXISTS "Subjects can be viewed by all" ON subjects;
DROP POLICY IF EXISTS "Authenticated can insert subjects" ON subjects;
DROP POLICY IF EXISTS "Chapters can be viewed by all" ON chapters;
...
-- Insert Information Technology - 402 subject (using 'elective' instead of 'optional')
...
BEGIN
    ...
    -- Insert Information Technology chapters (only if subject exists)
    IF it_subject_id IS NOT NULL THEN
    ...
    -- Insert Grammar chapters (only if subject exists)  
    IF grammar_subject_id IS NOT NULL THEN
        INSERT INTO chapters (subject_id, name, order_index, created_at) VALUES
